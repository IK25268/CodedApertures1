...
{
    BMP* bmp = bopen(fileName);
    Matrix* matrix = MatrixInit(bmp->width, bmp->height);
    for (unsigned int y = 0; y<matrix->rows; y++)
    {
        BstInsert(matrix->Pixels, NULL, ColsCreate(), &y);
        for (unsigned int x = 0; x<matrix->cols; x++)
        {
            Compl buf = NullC();
            buf.real = (bmp->pixels[y * bmp->width + x].red + bmp->pixels[y * bmp->width + x].blue + bmp->pixels[y * bmp->width + x].green) / 3.0f;
            BstInsert(BstFind(matrix->Pixels, &y), NULL, &buf, &x);
        }   
    }
    bclose(bmp);
    return matrix;
}
void SaveImg(void* matrix, const char* fileName)
{
    BMP* bmp = bopen(fileName);
    for (unsigned int y = 0; y<((Matrix*)matrix)->rows; y++)
    {
        for (unsigned int x = 0; x<((Matrix*)matrix)->cols; x++)
        {
            Tree* ytree = BstFind(((Matrix*)matrix)->Pixels, &y);
            Compl* xtree = BstFind(ytree, &x);
            bmp->pixels[y * bmp->width + x].red = (unsigned char)((*(Compl*)xtree).real);
            bmp->pixels[y * bmp->width + x].blue = 0;
            bmp->pixels[y * bmp->width + x].green = 0;
            bmp->pixels[y * bmp->width + x].alpha = 255;
        }  
    }
    for (unsigned int y = 0; y<((Matrix*)matrix)->rows; y++)
    {
        TreeItem* treeItem = BstFindItem(((Matrix*)matrix)->Pixels, &y);
        BstDestroy(treeItem->data, NULL, 0);
        //printf(" ");
    }
    BstDestroy(((Matrix*)matrix)->Pixels, NULL, 1);
    free(((Matrix*)matrix));
    bwrite(bmp, fileName);
    bclose(bmp);
}
...
// {
//     bmp->pixels = (pixel*) malloc(bmp->width * bmp->height * sizeof(pixel));
//     bmp->pixels = (Е*) malloc(bmp->width * bmp->height * sizeof(pixel));
//     _map(bmp, _get_pixel);
// }
